...
@Controller
@RequestMapping(value="/manager/backend")
public class AppCheckController {
	...
	public String getAppInfoList(Model model,HttpSession session,
		...
		List<AppCategory> categoryLevel1List = null;//列出一级分类列表
		...
		int totalCount = 0;
		try {
			totalCount = appService.getAppInfoCount(querySoftwareName,queryCategoryLevel1, 
		...
		try {
			appInfoList = appService.getAppInfoList(querySoftwareName, queryCategoryLevel1, 
					queryCategoryLevel2, queryCategoryLevel3, queryFlatformId, currentPageNo, 
					pageSize);
	...
	public List<AppCategory> getCategoryList(String pid){
		...
		try {
			categoryLevelList = appCategoryService.getAppCategoryListByParentId
					(pid == null?null:Integer.parseInt(pid));
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
		}
